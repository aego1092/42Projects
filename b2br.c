...
// possible. Pedagogy is effective only if peer evaluation is
// conducted seriously.
...
// During the defense, if you need assistance in verifying a point, the student being evaluated must assist you.
// Ensure that the "signature.txt" file is present at the root of the cloned repository.
// Check that the signature contained in "signature.txt" is identical to the signature of the virtual machine disk file with the ".vdi" extension. A simple "diff" should allow you to compare the two signatures. If necessary, ask the student being evaluated where their ".vdi" file is located.
// Note: the ".vdi" extension refers to VM disk files for VirtualBox. It is possible to use another VM provider (UTM), and the disk file will have a different extension.
// Verify that no snapshots exist. Most VM providers come with this feature.
// You can either: create a cold snapshot (before starting the VM) and erase it at the end of the evaluation, or duplicate the VM disk file in a different directory and boot from that copy. In both cases, this should leave the ".vdi" disk file untouched for the remaining evaluations, keeping the signature identical.
...
// Check that the README contains at least the required sections: Project description, OS choice (Debian or Rocky with pros/cons), main design choices (partitioning, security policies, user management, services).
// Check that the README also includes the additional required comparisons:
...
// The subject requests that a user with the login of the learner being evaluated is present
...
// Assign it a password of your choice, respecting the subject rules. The learner being evaluated must now explain to you how they were able to set up the rules requested in the subject on their virtual machine.
// Normally there should be one or two modified files. If there is any problem, the evaluation stops here.

// User step 2

// Now that you have a new user, ask the learner being evaluated to create a group named "evaluating" in
// front of you and assign it to this user. Finally, check that this user belongs to the "evaluating" group.

// Finally, ask the learner being evaluated to explain the advantages of this password policy, as well as the
...
// Check that the hostname of the machine is correctly formatted as follows: login42 (login of the learner being evaluated).
...
// Ask the learner being evaluated how to view the partitions for this virtual machine.
// Compare the output with the example given in the subject. Please note: if the learner evaluated makes the bonuses, it will be necessary to refer to the bonus example.
// This part is an opportunity to discuss the scores!
// The learner being evaluated should give you a brief explanation of how LVM works and what it is all about.
